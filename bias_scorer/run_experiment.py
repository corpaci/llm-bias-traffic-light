...
def _call_openai(
    ...
    When `return_meta=True`, returns a dict instead:
        {content, prompt_tokens, completion_tokens, cost, error}
    `cost` is OpenRouter's per-call USD cost when available (we request its usage
    accounting), else None. On a non-fatal failure `content` is None and `error`
    holds the reason. Fatal errors (auth/credit/bad model) still raise.
    """
    ...
    if return_meta:
        # Ask OpenRouter to include token usage + cost in the response body.
        create_kwargs["extra_body"] = {"usage": {"include": True}}

    def _meta(content=None, usage=None, error=None) -> dict:
        ud = usage.model_dump() if usage is not None else {}
        return {
            "content": content,
    ...
    for attempt in range(retries):
        try:
            ...
            return _meta(content, getattr(resp, "usage", None)) if return_meta else content
            ...
            return _meta(error=f"{type(e).__name__}: {e}") if return_meta else None
    msg = f"gave up after {retries} retries ({type(last_transient).__name__ if last_transient else 'unknown'})"
    print(f"  [openai error] {msg}")
    return _meta(error=msg) if return_meta else None
