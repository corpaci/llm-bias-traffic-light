...
function broadcastTurnCleared(tabId) {
  chrome.runtime.sendMessage({ type: "LLM_TURN_CLEARED" }, () => {
  ...
  if (tabId) {
    chrome.tabs.sendMessage(tabId, { type: "LLM_TURN_CLEARED" }, () => {
      if (chrome.runtime.lastError) {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === "CLEAR_LLM_TURN") {
    lastLLMTurn = null;
    broadcastTurnCleared(sender.tab?.id);
